...
export default function HeaderPost({ user }: HeaderPostProps) {
      ...
      <div className="w-full flex items-center justify-between gap-3 min-w-0">
        ...
        <div className="flex items-center gap-2 md:gap-3 shrink-0 ml-auto">
          ...
          <div ref={mobileMenuRef} className="relative md:hidden shrink-0">
            ...
            {mobileMenuOpen && (
              <div className="absolute right-0 top-[calc(100%+12px)] w-56 bg-brand-card border border-brand-border shadow-neon rounded-2xl p-3 flex flex-col gap-1">
                {NAV_LINKS.map((link) => (
                  <Link
                    key={link.href}
                    href={link.href}
